...
**⚠️ ملاحظة 2026-08-09:** جرت منذ هذا التاريخ عدة جولات فحص وإصلاح أمني/اقتصادي كبيرة غير موثَّقة هنا
(تحقق من `server.js`/`git log` للحالة الفعلية الحالية بدل الاعتماد على هذا الملف وحده). من ضمنها: حماية
شراء/ترقية الأسلحة، دخل الغياب، مكافآت التحالف، شجرة الترقيات، الإنجازات، البرستيج، الواحات، ومؤخراً
`landsState` (ترقية مباني القرى). راجع رسائل commit الأخيرة على `main` للتفاصيل الدقيقة.
