...
## 9. تحسينات تجربة المستخدم UX — 2026-07-06

### أ. تأكيد الصرف (Confirmation Dialog) — جديد
- **الملف:** `js/ui/ui-core.js` + `index.html` + `css/style.css`
- **الوظيفة:** نافذة منبثقة بتأكيد قبل أي صرف للجواهر 💎
- **دالة `showConfirmDialog(opts)`** — نافذة بتأكيد/إلغاء، ذيلها قصصي
- **دالة `spendWithConfirm(type, amount, label, onSpend)`** — صرف آمن مع تأكيد
- **التكامل:**
  - ترقية الأسلحة (يظهر التأكيد إذا الترقية تستهلك جواهر) — `ui-promotion.js:_upgradeWeapon`
  - صرف 💎←💰 في المتجر — `ui-promotion.js:exchange_gems_cash`
  - إعادة الميلاد (Prestige) — `ui-core.js:renderPrestige` — رسالة "الملك العظيم، هل أنت متأكد؟"
- **دالة `getBonusForLevel()`** في `prestige.js` لعرض تفاصيل البونص القادم في التأكيد

### ب. مؤشر الاتصال (Offline Indicator) — جديد
- **الملف:** `index.html` + `css/style.css` + `js/network-sync.js`
- **الوظيفة:** إظهار مؤشر أحمر "🌪️ عاصفة رملية قطعت الاتصال..." عند انقطاع WebSocket
- **آلية العمل:** دالة `_setOfflineIndicator(true/false)` في `network-sync.js` — تشتغل مع `onclose`/`onerror`/`onopen`
- **التصميم:** مؤشر مثبت أعلى الشاشة مع أنيميشن نبض ووميض، يختفي تلقائياً عند إعادة الاتصال

### ج. شاشة خطأ التحميل — جديد
- **الملف:** `index.html` + `js/main.js`
- **الوظيفة:** عند فشل تحميل اللعبة، تظهر رسالة "عذراً! تعذر تحميل اللعبة" مع زر إعادة المحاولة
- **آلية العمل:** دالة `showLoadingError()` في `main.js` تستدعى من `init().catch()`

### د. حالات فارغة (Empty States) — محسّن
- **الملف:** `js/ui/ui-core.js` + `css/style.css`
- **الوظيفة:** شاشات أنيقة بدلاً من الفراغ التام مع رسائل مرتبطة بالقصة
- **التغييرات:**
  - **المخزون:** "لم تصنع أي قطعة بعد. توجه إلى ورشة التصنيع!" (أنيقة بدلاً من نص بسيط)
  - **الأحداث:** "الصحراء هادئة اليوم... لكن التاجر يهمس بقدوم حدث قريب" (مرتبطة بالقصة)
  - **الواحات:** "لم تسيطر على أي واحة بعد. الواحات تدر دخلاً ثابتاً" (جديد — كان فاضي)
  - **المهام اليومية:** "كل المهام اكتملت! استعد غداً" (جديد — كان فاضي)
- **كلاس CSS:** `.empty-state` مع `.empty-state-icon`, `.empty-state-title`, `.empty-state-desc`, `.empty-state-cta`

### هـ. تحسين حجم اللمس والخط — شامل
- **الملف:** `css/style.css`
- **التغييرات:**
  - كل الأزرار: `min-height: 44px` (معيار وصول)
  - أزرار القائمة السفلية: `min-height: 52px`
  - أزرار المتجر: `padding: 10px 16px` (بدلاً من 5px 12px)
  - الخط العربي: `font-size: clamp(0.7rem, 2.5vw, 0.85rem)` — لا أقل من 0.7rem
  - `touch-action: manipulation` على كل الأزرار — إزالة تأخير النقر
  - `-webkit-tap-highlight-color: transparent` — إزالة الوميض الرمادي
  - إخفاء spinners من حقول الأرقام

### و. توسعة التدريب (Tutorial) — من 7 إلى 14 خطوة
- **الملف:** `js/tutorial.js`
- **الخطوات الجديدة:**
  - `shop` — 🛒 المتجر والصرف
  - `weapons` — 🗡️ مكتبة الأسلحة
  - `army` — ⚔️ ساحة الجيش
  - `inventory` — 📦 المخزون والتصنيع
  - `quests` — 📜 المهام والقصة
  - `events` — 🎊 الأحداث
  - `village` — 🏘️ القرى والانتقال
